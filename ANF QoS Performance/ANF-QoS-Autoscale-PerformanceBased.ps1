...
Script Purpose:
Allocate Azure NetApp Files Manual QoS volume throughput based on historical average throughput usage metrics.
...
- FSL uses the current manual pool throughput as the performance budget. The script does not purchase, increase, or decrease FSL pool throughput.
- Every managed volume receives the configured minimum throughput first. Remaining throughput is weighted by historical average throughput usage.
- If the throughput metric window has no data for the pool's volumes, allocation falls back to provisioned volume size.
...
3. SETTINGS:
   ...
   - ANF_ThroughputLookBackHours: Lookback window in hours for TotalThroughput metrics (int, default: 168)
#>
...
$throughputLookBackHours = Convert-AnfSettingToInt -Name "ANF_ThroughputLookBackHours" -Value (Get-AnfSetting -Name "ANF_ThroughputLookBackHours" -Default 168) -Minimum 1
...
Write-Output "=== ANF QoS Performance Configuration ==="
...
Write-Output "Throughput Metric Lookback: $throughputLookBackHours hour(s)"
...
function Get-AnfMetricAverageValues {
    ...
    $queryString = "&metricnames=$([uri]::EscapeDataString($MetricName))&timespan=$([uri]::EscapeDataString($timespan))&interval=PT5M&aggregation=Average"
...
function Get-AnfPerformancePlan {
    param(
    ...
    $totalThroughputMetric = [double](($Volumes | Measure-Object -Property ThroughputMetric -Sum).Sum)
    $sizeFallbackUsed = $false
    $weightPropertyName = "ThroughputMetric"
    $weightDenominator = $totalThroughputMetric

    if ($weightDenominator -le 0) {
        $sizeFallbackUsed = $true
        $weightPropertyName = "SizeGiB"
        $weightDenominator = [double](($Volumes | Measure-Object -Property SizeGiB -Sum).Sum)
        if ($weightDenominator -le 0) {
            throw "Total throughput metric and total volume size are both 0. Cannot calculate performance allocation."
        }
    }

    $remainingThroughput = $PoolThroughputBudgetMibps - $minimumTotal
    $planRows = foreach ($volume in $Volumes) {
        $weightValue = [double]$volume.$weightPropertyName
        $metricWeightPercentage = if ($weightDenominator -gt 0) { [math]::Round(($weightValue / $weightDenominator) * 100, 3) } else { 0 }
        $rawTarget = $MinimumThroughputPerVolume + (($remainingThroughput * $weightValue) / $weightDenominator)
        $floorTarget = [math]::Floor($rawTarget)
        [PSCustomObject]@{
            ShortName = $volume.ShortName
            VolumeId = $volume.VolumeId
            SizeGiB = $volume.SizeGiB
            ThroughputMetric = [math]::Round([double]$volume.ThroughputMetric, 3)
            MetricWeightPercentage = $metricWeightPercentage
            SizeFallbackUsed = $sizeFallbackUsed
    ...
    if ($remainingRoundingThroughput -gt 0) {
        $roundingOrder = @(
            $planRows | Sort-Object `
                @{ Expression = 'FractionalRemainder'; Descending = $true },
                @{ Expression = 'ShortName'; Ascending = $true }
        )
        foreach ($row in @($roundingOrder | Select-Object -First $remainingRoundingThroughput)) {
...
try {
    ...
    if ($isFlexibleServiceLevel) {
        Write-Output "FSL uses the current manual pool throughput as the performance budget: $poolThroughputBudgetMibps MiB/s"
    } else {
    ...
    $volumeInputs = foreach ($anfVolume in $anfVolumes) {
        $throughputMetricValues = @(Get-AnfMetricAverageValues -ResourceId $anfVolume.Id -MetricName 'TotalThroughput' -LookBackHours $throughputLookBackHours)
        $throughputMetric = 0
        if ($throughputMetricValues.Count -gt 0) {
            $throughputMetric = [math]::Round([double](($throughputMetricValues | Measure-Object -Average).Average), 3)
        }
    ...
    $finalData = @(Get-AnfPerformancePlan -Volumes $volumeInputs -PoolThroughputBudgetMibps $poolThroughputBudgetMibps -MinimumThroughputPerVolume $minimumThroughputPerVolume)
    ...
    $unallocatedRow = [PSCustomObject]@{
        ...
        SizeFallbackUsed = ($finalData | Select-Object -First 1).SizeFallbackUsed
    ...
    $displayData | Select-Object ShortName, SizeGiB, ThroughputMetric, MetricWeightPercentage, SizeFallbackUsed, CurrentThroughputMibps, NewThroughputMibps, NetChangeInThroughputMibps | Format-Table -AutoSize

    if (($finalData | Select-Object -First 1).SizeFallbackUsed) {
        Write-Output "No TotalThroughput metric data was available in the lookback window. Allocation fell back to provisioned volume size."
    }
    ...
    if ($updates.Count -eq 0) {
        Write-Output "All volumes in pool '$anfPoolName' are already at the performance throughput values."
        continue
    }
...
if ($failedCapacityPools.Count -gt 0) {
    ...
    throw "ANF QoS Performance failed for $($failedCapacityPools.Count) pool(s)."
}

Write-Output "ANF QoS Performance completed."
