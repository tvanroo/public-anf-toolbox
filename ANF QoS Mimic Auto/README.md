...
The runbook calculates a per-pool throughput budget, assigns each volume a configurable minimum throughput floor, and distributes the remaining throughput in proportion to each volume's provisioned size. The result is a Manual QoS allocation that mimics the classic Auto QoS proportional behavior while keeping the control surface explicit and auditable.
...
For Flexible Service Level pools, capacity and throughput are independent. FSL uses the current manual pool throughput as the mimic-auto budget. This script does not purchase, increase, or decrease FSL pool throughput; it only allocates the throughput currently available on the pool across the volumes.
...
- Remaining pool throughput is allocated by provisioned volume size.
