...
$scriptPath = Join-Path $repoRoot 'ANF QoS Mimic Auto/ANF-QoS-Autoscale-MimicAuto.ps1'
$readmePath = Join-Path $repoRoot 'ANF QoS Mimic Auto/README.md'
...
if ($parseErrors.Count -gt 0) {
    throw "PowerShell parser found errors in ANF-QoS-Autoscale-MimicAuto.ps1: $($parseErrors.Message -join '; ')"
}
...
Assert-Contains -Haystack $scriptText -Needle 'FSL uses the current manual pool throughput as the mimic-auto budget' -Message 'Expected FSL behavior to use current pool throughput, not capacity-derived throughput.'
