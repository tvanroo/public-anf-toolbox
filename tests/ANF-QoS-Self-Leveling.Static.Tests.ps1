...
Assert-Contains -Haystack $scriptText -Needle 'function Update-AnfPoolSize' -Message 'Expected classic pool capacity update helper for automatic throughput expansion.'
Assert-Contains -Haystack $scriptText -Needle 'function Get-AnfClassicPoolSizingForThroughput' -Message 'Expected classic pool sizing helper to calculate capacity from required throughput.'
Assert-Contains -Haystack $scriptText -Needle 'function Wait-AnfPoolThroughputBudget' -Message 'Expected pool throughput wait helper before applying volume increases.'
Assert-Contains -Haystack $scriptText -Needle 'UnusedThroughputShareMibps' -Message 'Expected unused throughput to be redistributed into managed volume targets.'
Assert-Contains -Haystack $scriptText -Needle 'Excluded volume throughput reserved' -Message 'Expected excluded volume throughput to be reserved in planning output.'
...
Assert-Contains -Haystack $readmeText -Needle 'Excluded volumes are never changed by this runbook' -Message 'Expected README to document excluded volume reservation behavior.'
Assert-Contains -Haystack $readmeText -Needle 'Managed volumes consume the full remaining throughput budget' -Message 'Expected README to document full-budget managed allocation behavior.'
Assert-Contains -Haystack $readmeText -Needle 'expands the pool budget before increasing volume throughput' -Message 'Expected README to document automatic pool budget expansion.'
...
Assert-Contains -Haystack $readmeText -Needle "public-anf-toolbox%2F$mainBranchPath%2FANF%2520QoS%2520Self%2520Leveling%2Fdeploy%2Fazuredeploy.json" -Message 'Expected commercial deploy button to point at the main branch template.'
Assert-Contains -Haystack $deployText -Needle "raw.githubusercontent.com/tvanroo/public-anf-toolbox/$mainBranchPath/ANF%20QoS%20Self%20Leveling/ANF-QoS-Autoscale-SelfLeveling.ps1" -Message 'Expected deploy template to import the runbook from the main branch.'
Assert-Contains -Haystack $deployGovText -Needle "raw.githubusercontent.com/tvanroo/public-anf-toolbox/$mainBranchPath/ANF%20QoS%20Self%20Leveling/deploy/azuredeploy.json" -Message 'Expected Azure Gov wrapper to link the main branch shared template.'
