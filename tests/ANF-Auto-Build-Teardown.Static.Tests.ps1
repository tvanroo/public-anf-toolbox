...
Assert-Contains -Haystack $scriptText -Needle 'ANF_DeleteResourceGroupIfEmpty" -Default "Yes"' -Message 'Expected resource group deletion to default to guarded empty-only cleanup.'
...
Assert-Contains -Haystack $scriptText -Needle 'function Remove-AnfVolumesInPool' -Message 'Expected refreshed volume deletion helper for ordered deletes.'
Assert-Contains -Haystack $scriptText -Needle 'function Remove-AnfCapacityPoolWithNestedVolumeRetry' -Message 'Expected capacity pool delete retry helper for nested-volume delete lag.'
Assert-Contains -Haystack $scriptText -Needle 'function Get-AnfNestedVolumeResourceIdsFromError' -Message 'Expected nested volume Resource IDs to be parsed from Azure CannotDeleteResource errors.'
Assert-Contains -Haystack $scriptText -Needle "CannotDeleteResource|nested resources" -Message 'Expected pool delete to retry when Azure reports nested resources still exist.'
Assert-Contains -Haystack $scriptText -Needle "Capacity pool delete was blocked because Azure still reports nested volume resources" -Message 'Expected warning when pool delete is blocked by nested volume lag.'
...
Assert-Contains -Haystack $scriptText -Needle '-AccountPoolsResourceId $accountPoolsResourceId' -Message 'Expected delete path to pass the account pool collection into the retry helper before deleting the account.'
...
Assert-Contains -Haystack $scriptText -Needle 'function Remove-AnfResourceGroupIfEmpty' -Message 'Expected guarded empty-resource-group delete helper.'
Assert-Contains -Haystack $scriptText -Needle 'Skipping resource group deletion because it still contains resource(s)' -Message 'Expected non-empty resource groups to be preserved with a warning.'
Assert-Contains -Haystack $scriptText -Needle 'Get-AnfResourceGroupResources' -Message 'Expected resource group contents to be listed before resource group deletion.'
...
Assert-Contains -Haystack $readmeText -Needle '| `ANF_DeleteResourceGroupIfEmpty` | `Yes` |' -Message 'Expected README to document guarded resource group deletion default.'
Assert-Contains -Haystack $readmeText -Needle 'If other resources remain, the script warns and leaves the resource group intact' -Message 'Expected README to document non-empty resource group preservation.'
