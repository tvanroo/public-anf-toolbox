...
$scriptPath = Join-Path $repoRoot 'ANF QoS Volume Equity/ANF-QoS-Autoscale-VolumeEquity.ps1'
$readmePath = Join-Path $repoRoot 'ANF QoS Volume Equity/README.md'
$deployPath = Join-Path $repoRoot 'ANF QoS Volume Equity/deploy/azuredeploy.json'
$deployGovPath = Join-Path $repoRoot 'ANF QoS Volume Equity/deploy/azuredeploy-gov.json'
$deployGovBadgePath = Join-Path $repoRoot 'ANF QoS Volume Equity/deploy/deploytoazuregov.svg'
$behaviorImagePath = Join-Path $repoRoot 'ANF QoS Volume Equity/media/qos-volume-equity-behavior.png'
...
if ($parseErrors.Count -gt 0) {
    throw "PowerShell parser found errors in ANF-QoS-Autoscale-VolumeEquity.ps1: $($parseErrors.Message -join '; ')"
}
...
Assert-Contains -Haystack $scriptText -Needle 'FSL uses the current manual pool throughput as the volume-equity budget' -Message 'Expected FSL behavior to use current pool throughput, not capacity-derived throughput.'
Assert-Contains -Haystack $scriptText -Needle 'function Get-AnfVolumeEquityPlan' -Message 'Expected explicit volume equity allocation helper.'
Assert-Contains -Haystack $scriptText -Needle 'Every managed volume receives an equal share of the pool throughput budget' -Message 'Expected script header to document equal-share behavior.'
...
Assert-Contains -Haystack $readmeText -Needle 'media/qos-volume-equity-behavior.png' -Message 'Expected README to include the QoS Volume Equity behavior graphic.'
...
Assert-Contains -Haystack $readmeText -Needle 'equal share of the pool throughput budget' -Message 'Expected README to document equal-share allocation behavior.'
...
if (-not (Test-Path -LiteralPath $behaviorImagePath)) {
    throw 'Expected QoS Volume Equity behavior graphic to exist.'
}

Write-Output 'ANF-QoS-Volume-Equity static checks passed.'
