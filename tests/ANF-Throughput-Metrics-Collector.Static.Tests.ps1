...
Assert-Contains -Haystack $scriptText -Needle 'Read-Host "Enter subscription number"' -Message 'Expected local runs to prompt for a subscription when multiple are visible.'
Assert-Contains -Haystack $scriptText -Needle 'Write-Host "Multiple active Azure subscriptions are visible. Select one for ANF discovery:"' -Message 'Expected subscription picker list header to be written to the host, not captured as function output.'
Assert-Contains -Haystack $scriptText -Needle 'Write-Host ("  [{0}] {1} ({2})"' -Message 'Expected subscription picker options to be written to the host, not captured as function output.'
