...
Assert-Contains -Haystack $readmeText -Needle '$DownloadStamp = (Get-Date).ToUniversalTime().ToString("yyyyMMdd-HHmmssZ")' -Message 'Expected README quick start to use a timestamped download name.'
Assert-Contains -Haystack $readmeText -Needle 'ANF-throughput-metrics-collector-$DownloadStamp.ps1' -Message 'Expected README quick start to avoid stale local script reuse.'
