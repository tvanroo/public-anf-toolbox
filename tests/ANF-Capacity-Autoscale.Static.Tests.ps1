...
Assert-Contains -Haystack $scriptText -Needle 'function Get-AnfMetricAverageValues' -Message 'Expected capacity metrics to be read through ARM REST instead of Az.Monitor cmdlets.'
Assert-NotContains -Haystack $scriptText -Needle 'Az.NetAppFiles' -Message 'Expected script not to require the Az.NetAppFiles module in the PowerShell 7.x runtime.'
Assert-NotContains -Haystack $scriptText -Needle 'Az.Monitor' -Message 'Expected script not to require the Az.Monitor module in the PowerShell 7.x runtime.'
...
Assert-Contains -Haystack $scriptText -Needle 'function Convert-AnfJsonObjectToHashtable' -Message 'Expected JSON object conversion helper that works without PowerShell 7-only ConvertFrom-Json switches.'
...
Assert-Contains -Haystack $scriptText -Needle 'Get-AnfPool -SubscriptionId $subscriptionId' -Message 'Expected ANF pool lookup to use the parsed target subscription ID.'
Assert-Contains -Haystack $scriptText -Needle 'Get-AnfVolumes -SubscriptionId $subscriptionId' -Message 'Expected ANF volume lookup to use the parsed target subscription ID.'
Assert-Contains -Haystack $scriptText -Needle 'Update-AnfPoolSize -SubscriptionId $subscriptionId' -Message 'Expected ANF pool resize to use the parsed target subscription ID.'
Assert-NotContains -Haystack $scriptText -Needle '/subscriptions/$($context.Subscription.Id)/resourceGroups/$ResourceGroupName/providers/Microsoft.NetApp' -Message 'Expected ANF REST paths not to be built from the active Az context subscription.'
...
Assert-Contains -Haystack $scriptText -Needle 'Disable-AzContextAutosave -Scope Process' -Message 'Expected runbook to disable Az context autosave before managed identity authentication.'
...
Assert-Contains -Haystack $scriptText -Needle 'Volume contraction targets MaxConsumedSizeGiB + ANF_MinimumFreeSpaceGiB without extra utilization or free-space gates.' -Message 'Expected script header to document unconstrained contraction target behavior.'
Assert-Contains -Haystack $scriptText -Needle 'Pool expands or contracts to the minimum whole-TiB size required by planned volume targets' -Message 'Expected script header to document direct pool expansion/contraction target behavior.'
Assert-NotContains -Haystack $scriptText -Needle 'capacityResizeThreshold - 15' -Message 'Expected volume contraction not to require an extra 15 percentage point utilization buffer.'
Assert-NotContains -Haystack $scriptText -Needle 'minimumFreeSpaceGiB * 3' -Message 'Expected volume contraction not to require a 3x minimum-free-space gate.'
Assert-NotContains -Haystack $scriptText -Needle 'full TiB can be freed' -Message 'Expected script header not to describe removed pool contraction gating.'
...
Assert-Contains -Haystack $readmeText -Needle '| Volume contraction target | `MaxConsumedSizeGiB + ANF_MinimumFreeSpaceGiB` |' -Message 'Expected README hard-coded decision table to document direct volume contraction target behavior.'
Assert-NotContains -Haystack $readmeText -Needle 'Contraction utilization buffer' -Message 'Expected README not to document the removed contraction utilization buffer.'
Assert-NotContains -Haystack $readmeText -Needle 'Contraction free-space gate' -Message 'Expected README not to document the removed contraction free-space gate.'
...
Assert-NotContains -Haystack $deployText -Needle '"Az.NetAppFiles"' -Message 'Expected commercial deploy template not to import Az.NetAppFiles into the PowerShell 7.2 runtime.'
Assert-NotContains -Haystack $deployText -Needle '"Az.Monitor"' -Message 'Expected commercial deploy template not to import Az.Monitor into the PowerShell 7.2 runtime.'
Assert-Contains -Haystack $deployText -Needle '"defaultValue": "[concat(''ANF-Capacity-Autoscale-'', take(uniqueString(resourceGroup().id, deployment().name), 6))]"' -Message 'Expected commercial deploy template to provide a relevant unique default Automation Account name.'
...
Assert-Contains -Haystack $deployText -Needle '"targetAnfAccountResourceId"' -Message 'Expected commercial deploy template to derive the target ANF account Resource ID from the capacity pool Resource ID.'
Assert-Contains -Haystack $deployText -Needle '"targetAnfResourceGroupName"' -Message 'Expected commercial deploy template to parse the target ANF resource group from the capacity pool Resource ID.'
Assert-Contains -Haystack $deployText -Needle '"subscriptionId": "[variables(''targetAnfSubscriptionId'')]"' -Message 'Expected RBAC nested deployment to target the ANF subscription, not the deployment subscription.'
Assert-Contains -Haystack $deployText -Needle '"resourceGroup": "[variables(''targetAnfResourceGroupName'')]"' -Message 'Expected RBAC nested deployment to target the ANF resource group, not the Automation Account deployment resource group.'
Assert-Contains -Haystack $deployText -Needle '"scope": "[format(''Microsoft.NetApp/netAppAccounts/{0}'', parameters(''targetAnfAccountName''))]"' -Message 'Expected RBAC assignments to be scoped to the target ANF account inside the target ANF resource group.'
...
Assert-Contains -Haystack $readmeText -Needle 'target ANF account scope' -Message 'Expected README to document target account scoped RBAC.'
Assert-Contains -Haystack $readmeText -Needle 'roleAssignments/write' -Message 'Expected README to document required permission for automatic RBAC assignment.'
Assert-Contains -Haystack $deployText -Needle '"runbookType": "PowerShell72"' -Message 'Expected commercial deploy template to create the runbook on the PowerShell 7.2 runtime.'
...
Assert-Contains -Haystack $deployGovText -Needle '"defaultValue": "[concat(''ANF-Capacity-Autoscale-'', take(uniqueString(resourceGroup().id, deployment().name), 6))]"' -Message 'Expected Azure Gov deploy template to provide a relevant unique default Automation Account name.'
