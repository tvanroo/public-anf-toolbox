...
Assert-Contains -Haystack $deployText -Needle '"scope": "[variables(''targetAnfAccountResourceId'')]"' -Message 'Expected RBAC assignments to be scoped to the target ANF account, not the deployment resource group.'
Assert-Contains -Haystack $deployText -Needle '"roleAssignmentPrincipalGuid"' -Message 'Expected RBAC assignment names to be generated from target scope, principal, and role.'
