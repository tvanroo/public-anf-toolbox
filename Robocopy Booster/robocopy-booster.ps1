...
Script Purpose:
This script runs parallel Robocopy commands as PowerShell jobs to speed up many-small-file migrations.
...
Write-Host '=== Robocopy Booster - Configuration ===' -ForegroundColor Cyan
...
$copyJobScript = {
    ...
    & $RobocopyPath @robocopyArguments | ForEach-Object { Write-Host $_ }
    $exitCode = if ($null -eq $LASTEXITCODE) { 0 } else { [int]$LASTEXITCODE }

    $color = if ($exitCode -ge 8) { 'Red' } elseif ($exitCode -ge 4) { 'Yellow' } else { 'Green' }
    Write-Host "Completed: $JobName (Robocopy exit code: $exitCode)" -ForegroundColor $color
...
foreach ($workItem in $workItems) {
    ...
    $jobName = "RobocopyBooster-$($workItem.Name)-$([guid]::NewGuid().ToString('N').Substring(0, 8))"
...
Write-Host ''
Write-Host '=== Robocopy Booster - Results ===' -ForegroundColor Cyan
